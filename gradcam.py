...
class GradCam():
    @classmethod
    def from_interp(cls,learn,target_layer, interp,img_idx, ds_type=DatasetType.Valid,include_label=False):
        ...
        hmap_pred,xb_grad_pred = get_grad_heatmap(learn,target_layer, xb,pred_idx,size=xb_img.shape[-1])
        ...
        if include_label:
            ...
            if actual_idx!=pred_idx:
                hmap_actual,xb_grad_actual = get_grad_heatmap(learn,target_layer, xb,actual_idx,size=xb_img.shape[-1])
    ...
    @classmethod
    def from_one_img(cls,learn,target_layer, x_img,label1=None,label2=None):
        '''
        ...
        hmap1,xb_grad1 = get_grad_heatmap(learn,target_layer, xb,label1_idx,size=xb_img.shape[-1])
        prob1 = probs[label1_idx]
        
        label2_args = None
        if label2:
            label2_idx = learn.data.classes.index(label2)
            hmap2,xb_grad2 = get_grad_heatmap(learn,target_layer, xb,label2_idx,size=xb_img.shape[-1])
...
def get_grad_heatmap(learn,target_layer, xb,y,size):
    '''
